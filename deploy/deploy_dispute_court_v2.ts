...
async function main(): Promise<void> {
  ...
  if (!rawKey) {
    throw new Error(
      ...
      "  export GENLAYER_PRIVATE_KEY=$(grep GENLAYER_PRIVATE_KEY .env.local | cut -d= -f2 | tr -d '[:space:]')"
    );
  }
  ...
  const contractPath = path.resolve(process.cwd(), 'contracts/dispute_court_v2.py');
  const code = new Uint8Array(readFileSync(contractPath));

  console.log('Deploying DisputeCourt v2 contract to testnet-bradbury…');
  ...
  console.log('\n✓ DisputeCourt v2 deployed successfully on testnet-bradbury!');
  console.log(`  Contract address : ${contractAddress}`);
  console.log(`  Deploy tx hash   : ${deployTx}`);
  console.log('\nAdd to your .env.local:');
  console.log(`  NEXT_PUBLIC_DISPUTE_COURT_V2_ADDRESS=${contractAddress}`);
}
